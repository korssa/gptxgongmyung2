...
export function GoogleTranslateWidget() {
  ...
  useEffect(() => {
    ...
    // ====== 2) 콤보 옵션 업데이트 (수정: English 강제 선택 및 우선순위) ======
    function updateLanguageOptions() {
      const combo = document.querySelector(".goog-te-combo") as HTMLSelectElement | null;
      if (!combo || !combo.options) return;

      const { countryByLang, nativeByLang, includedLanguages } = buildMaps() as any;
      ...
      // ✅ 우선순위 옵션 먼저 추가 (그리고 영어는 기본 선택으로 강제)
      prioritizedOptions.forEach((opt) => combo.appendChild(opt));

      // 영어(en)를 무조건 기본 선택
      const enOpt = prioritizedOptions.find((o) => normalizeCode(o.value) === "en");
      if (enOpt) {
        enOpt.selected = true;
        combo.value = "en";
      } else {
          ...
          new (window.google as any).translate.TranslateElement(
            {
              ...
              autoDisplay: false,
              layout: (window.google as any).translate.TranslateElement?.InlineLayout?.HORIZONTAL || "horizontal",
            },
